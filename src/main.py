...
# Static floor
#Static floor (sàn tĩnh) là một phần của môi trường vật lý trong game, được tạo ra bằng pymunk để làm nền cho các vật thể khác tương tác với nó.
...
def sling_action():
    ...
    global x_mouse
    global y_mouse
    # Fixing bird to the sling rope (cố định bird vào dây kéo)
    ...
    # Angle of impulse (góc bắn của bird khi rời khỏi dây kéo)
    dy = y_mouse - sling_y
    dx = x_mouse - sling_x
    if dx == 0:
        dx = 0.00000000000001
...
# bird and pigs (Khi chim (collision type = 0) va chạm với lợn (collision type = 1), hàm post_solve_bird_pig() sẽ xử lý va chạm)
space.add_collision_handler(0, 1).post_solve=post_solve_bird_pig
# bird and wood ( Khi chim (0) đâm vào gỗ (2), hàm post_solve_bird_wood() sẽ được gọi để kiểm tra mức độ tác động và có thể làm gỗ vỡ)
space.add_collision_handler(0, 2).post_solve=post_solve_bird_wood
# pig and wood (Khi lợn (1) đập vào gỗ (2), hàm post_solve_pig_wood() sẽ kiểm tra xem lực tác động có đủ mạnh để làm lợn mất máu hay không)
...
while running:
    # Input handling (nơi xảy ra các sự kiện của game)
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # Toggle wall (phím A -> Bật tắt bức tường vật lí của game)
            if wall:
                for line in static_lines1:
                    space.remove(line)
                wall = False
        ...
        # Phím S → Chế độ không trọng lực (mọi vật thể rơi chậm hơn).
        # Phím N → Trở lại trọng lực bình thường (vật thể rơi nhanh như bình thường).
...
#đoạn này dùng để bắn 2 con một lúc (new update)
#-----------------------------------------------------------------------------------------------------
        elif event.type == pygame.KEYDOWN and event.key == pygame.K_d:
            # Release new birds (modified to shoot two birds) (xử lí bắn 2 chim cùng 1 lúc)
            mouse_pressed = False
            if level.number_of_birds > 0:
                ...
                if x_mouse < sling_x + 5:
                    bird1 = Bird(mouse_distance, angle, xo, yo, space)
                    bird2 = Bird(mouse_distance * 0.9, angle + 0.1, xo, yo, space)  # Adjust second bird slightly (chim 2 bay hơi lệch với chim 1)
                else:
                    bird1 = Bird(-mouse_distance, angle, xo, yo, space)
                    bird2 = Bird(-mouse_distance * 0.9, angle + 0.1, xo, yo, space)
